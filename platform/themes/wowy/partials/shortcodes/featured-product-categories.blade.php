<section class="popular-categories section-padding-60" id="featured-product-categories">
    <div class="container wow fadeIn animated">
        ...
        <div class="carousel-6-columns-cover position-relative ">
            <div class="slider-arrow slider-arrow-2 carousel-6-columns-arrow" id="carousel-6-columns-categories-arrows">
            </div>
            <div class="carousel-slider-wrapper  carousel-6-columns" id="carousel-6-columns-categories"
                data-slick="{{ json_encode([
                    ...
                    'autoplaySpeed' => (int) (in_array($shortcode->autoplay_speed, theme_get_autoplay_speed_options())
                        ? $shortcode->autoplay_speed
                        : 3000),
                    'speed' => 800,
                ]) }}">
                @foreach ($categories as $category)
                    <div class="card-1 border-radius-10 hover-up p-20 mt-20" style="background: white !important">
                        ...
                        <h5><a style="font-weight: bolder" href="{{ $category->url }}">{{ $category->name }}</a></h5>
                    </div>
                @endforeach
            </div>
        </div>
